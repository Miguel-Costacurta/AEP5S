...
form.addEventListener("submit", function (evento) {
    ...
    if (usuario) {
            ...
            document.getElementById("formCadastroFuncionario").style.display = "block";
            mostrarPendentes();
        } else {
...
function mostrarPendentes() {
    ...
    pendentes.forEach(function (s) {
            ...
            html = html + "</select>";
            html = html + "<button data-cancelar='" + s.protocolo + "'>Cancelar</button>";
            html = html + "</div>";
        });
...
listaPendentes.addEventListener("click", function (evento) {
    const protocolo = evento.target.getAttribute("data-cancelar");
...
formCadastroFuncionario.addEventListener("submit", function (evento) {
    evento.preventDefault();
    ...
    if (nome === "" || email === "" || senha === "") {
        resultadoCadastro.textContent = "Preencha todos os campos.";
        return;
    }
    ...
    resultadoCadastro.textContent = "Funcionário cadastrado: " + nome + " (" + tipo + ")";
});
